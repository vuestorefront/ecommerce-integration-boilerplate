...
`useUserShipping` comes with a set of getters that give you easy access to commonly used properties.
...
export default {
  setup () {
    const { search } = useUserShipping('<UNIQUE_ID>');
